...
- **テクスチャ背景**: シーンカードや背景に紙や黒板のような質感を適用できる設定を追加。
- **章のカラーカスタマイズ**: 章ごとに色を設定し、シーン一覧で色分け表示が可能。
- **モダンなUIスイッチ**: 設定画面のチェックボックスを直感的なトグルスイッチに刷新。
- **執筆完了スタンプ**: 本文からセパレーターを消したシーンに「執筆済み」の赤いスタンプを表示。
- **モバイル版書き出しの改善**: Android/iOSでプロジェクト名のフォルダへ自動で書き出す機能。
